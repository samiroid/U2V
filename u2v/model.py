...
class User2Vec(nn.Module):
    ...
    def __init__(self, user_id, encoder_id, run_id, emb_dimension, outpath, logs_path=None,
        ...
        if logs_path:
            pass
            # self.tensorboard = SummaryWriter(logs_path)
        self.run_id = run_id
    ...
    def fit(self, X_positive, X_negative):        
        ...
        print("User {} | tr: {} ({}) | val: {}".format(self.user_id, N, n_windows, n_val))        
        ...
        with trange(self.epochs) as pbar:
            # pbar.set_description("User {} | tr: {} ({}) | val: {}".format(self.user_id, N, n_windows, n_val))
            for e in pbar: #range(self.epochs):                    
                running_loss = 0.0       
                ...
                if self.training_split < 1: #validation performance                    
                    ...
                    if self.tensorboard:                    
                        self.tensorboard.add_scalars("loss/train", {self.user_id:avg_loss} , e)
                        self.tensorboard.add_scalars("loss/val", {self.user_id:val_prob}, e)
                        
                        # self.tensorboard.add_scalar(f"train/{self.user_id}", avg_loss , global_step)
                        # self.tensorboard.add_scalar(f"val/{self.user_id}", val_prob , global_step)
                        # self.tensorboard.add_scalar("loss/val", {self.user_id:val_prob}, e)
                else:
                    # self.tensorboard.add_scalar("loss/train", {self.user_id:avg_loss} , e)         
    ...
    def save_embedding(self): 
        ...
        # else:
        #     fname = f"{self.outpath}/{self.encoder_id}_{self.user_id}.txt"
